...
public static class RepositoryExtension
{
    ...
    /// <returns></returns>
    public static int FakeDelete<T>(this ISqlSugarClient db, List<long> ids) where T : EntityBase, new()
    {
        return db.Updateable<T>()
            .SetColumns(x => new T { IsDelete = true},appendColumnsByDataFilter:true)
            .Where(x => ids.Contains(x.Id))
            .ExecuteCommand();
    }

    public static async Task<int> FakeDeleteAsync<T>(this ISqlSugarClient db, List<long> ids) where T : EntityBase, new()
    {
    ...
    /// <param name="ids"></param>
    /// <returns></returns>
    public static async Task<int> FakeDeleteAsync<T>(this ISugarRepository repository, List<long> ids) where T : EntityBase, new()
    {
        return await repository.Context.FakeDeleteAsync<T>(ids);
    }
    ...
    /// <param name="ids"></param>
    /// <returns></returns>
    public static int FakeDelete<T>(this ISugarRepository repository, List<long> ids) where T : EntityBase, new()
    {
        return repository.Context.FakeDelete<T>(ids);
    }
    ...
    public static ISugarQueryable<T> OrderBuilder<T>(this ISugarQueryable<T> queryable, BasePageInput pageInput, string prefix = "", string defaultSortField = "Id", bool descSort = true)
    {
        ...
        if (!string.IsNullOrEmpty(nowPagerInput.Field) && !string.IsNullOrEmpty(nowPagerInput.Order))
        {
            ...
            orderStr = col != null
                ? $"{prefix}{col.DbColumnName} {(nowPagerInput.Order.ToLower() == nowPagerInput.DescStr ? "Desc" : "Asc")}"
                : $"{prefix}{nowPagerInput.Field} {(nowPagerInput.Order.ToLower() == nowPagerInput.DescStr ? "Desc" : "Asc")}";
        }
