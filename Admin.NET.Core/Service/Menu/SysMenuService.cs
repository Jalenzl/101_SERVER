...
public class SysMenuService : IDynamicApiController, ITransient
{
    ...
    public async Task<List<MenuOutput>> GetLoginMenuTree()
    {
        ...
        else
        {
            ...
            //默认增加首页
            var currentList = await _sysMenuRep.AsQueryable().Where(x => x.Pid == 0 && x.Title.Contains("首页")).ToListAsync();
            var currentIdList = currentList.Select(x => x.Id).ToList();
            menuIdList.AddRange(currentIdList);
            menuIdList= menuIdList.Distinct().ToList(); // 去重
    ...
    public async Task UpdateMenu(UpdateMenuInput input)
    {
        ...
        if(input.Type != MenuTypeEnum.Btn) 
        {
            var isExist = await _sysMenuRep.IsAnyAsync(u => u.Title == input.Title && u.Type == input.Type && u.Id != input.Id);
            if (isExist)
                throw Oops.Oh(ErrorCodeEnum.D4000);
        }                    
