...
public class SysOrgService : IDynamicApiController, ITransient
{
    ...
    #region add by cong 20250808 加载送试方
    /// <summary>
    /// 获取送试方数据
    /// </summary>
    /// <returns></returns>
    [DisplayName("获取送试方数据")]
    public async Task<List<OrgOutput>> GetPartyAList()
    {
        string partyACode = "songshifang";
        long pid = 0;
        var org = await GetSysOrg(new OrgInput { Code = partyACode });
        ...
        var result = await _sysOrgRep.AsQueryable().Where(x => x.Pid == pid).OrderBy(x => x.OrderNo).Select(x => new OrgOutput
                    {
                        OrgId = x.Id,
                        OrgName = x.Name
                    }).ToListAsync();
    ...
    /// <returns></returns>
    [DisplayName("获取指定机构")]
    public async Task<SysOrg> GetSysOrg(OrgInput input)
    {
        var result = await _sysOrgRep.AsQueryable()
                   .Where(x => x.Pid == 0 && x.Code == input.Code)
                   .OrderBy(u => u.OrderNo).FirstAsync();
        return result;
    }
